...
Phase 23 records the implementation contract for possible future runtime
deprecation warnings on the six validation-helper exports selected in
`.planning/API-EXPORT-CLEANUP-RFC.md`.

This document is design-only. It does not remove exports, add warnings, edit
source code, edit tests, change user-facing docs, or alter Abacus parity
evidence.
...
The six public validation helpers are not isolated leaf APIs today. They are
called from supported constructors and builders:

- `CalibrationStepConfig` calls `validate_calibration_step_config`.
...
- typed config/data constructors call `validate_sampler_config`,
  `validate_model_config`, and `validate_mmm_data`.

Therefore a future implementation must not add `Base.depwarn` directly to the
current validator bodies. That would warn during ordinary constructor and
builder use, punishing the migration path we are telling users to adopt. Tiny
change, large footgun.

## Future Implementation Pattern

Use manual `Base.depwarn` wrappers rather than `@deprecate`. The future public
functions should keep their current names and signatures, emit one deprecation
warning when directly called, then delegate to warning-free internal helpers.
This preserves behaviour while giving tests and docs precise control over the
warning boundary.
...
- Move the current validation body in `src/mmm/calibration.jl` to
  `_validate_calibration_step_config`.
- Update `CalibrationStepConfig` construction to call
  `_validate_calibration_step_config`.
- Keep `validate_calibration_step_config` as the public warning wrapper.
...
- Update `build_lift_test_calibration_payload` and any internal payload
  validation path to call `_validate_lift_test_calibration_payload`.
- Keep `validate_lift_test_calibration_payload` as the public warning wrapper.
...
- Update `build_cost_per_target_calibration_payload` and any internal payload
  validation path to call `_validate_cost_per_target_calibration_payload`.
- Keep `validate_cost_per_target_calibration_payload` as the public warning
  wrapper.
...
- Move the current validation body in `src/model/types.jl` to
  `_validate_mmm_data`.
- Update `MMMData` construction to call `_validate_mmm_data`.
- Keep `validate_mmm_data` as the public warning wrapper.
...
- Update `ModelConfig` construction and loader paths to call
  `_validate_model_config`.
- Keep `validate_model_config` as the public warning wrapper.
...
- Update `SamplerConfig` construction and loader paths to call
  `_validate_sampler_config`.
- Keep `validate_sampler_config` as the public warning wrapper.
...
A later runtime implementation phase should use targeted tests, not the full
suite by default. Expected lanes:
...
## Future Documentation Timing

Do not announce runtime deprecation in user-facing docs until runtime warnings
actually land. At that point:
