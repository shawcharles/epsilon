# Geo-Brand Panel Demo

This bundle demonstrates Epsilon's bounded panel config shape with two panel
axes: `geo` and `brand`.

Files:

- `config.yml`: `PanelMMM` configuration with
  `dimensions.panel = ["geo", "brand"]`.
- `dataset.csv`: demo panel data with date, geo, brand, revenue, and media
  channels.
