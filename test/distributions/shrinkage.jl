...
@testset "HorseshoePrior" begin
    ...
    @test coeffs ≈ [0.25, -1.5] atol = 1e-12 rtol = 1e-12
...
@testset "FinnishHorseshoePrior" begin
    ...
    @test coeffs ≈ 1.5 .* [lambda_tilde[1] * 2.0, -lambda_tilde[2] * 2.0] atol = 1e-12 rtol = 1e-12
...
@testset "R2D2Prior" begin
    ...
    @test variances ≈ [4.5, 13.5] atol = 1e-12 rtol = 1e-12

    coeffs = r2d2_coefficients(prior, [2.0, -1.0], [1.0, 3.0], 2.0)
    @test coeffs ≈ [2.0 * sqrt(4.5), -sqrt(13.5)] atol = 1e-12 rtol = 1e-12
