...
const _GEO_PANEL_FIXTURE_DIR = joinpath(@__DIR__, "..", "fixtures", "golden", "geo_panel")

@testset "geo_panel golden fixture config/data spine" begin
    fixture = GOLDEN_GEO_PANEL_CONFIG_DATA
    config_path = joinpath(_GEO_PANEL_FIXTURE_DIR, "config.yml")
    dataset_path = joinpath(_GEO_PANEL_FIXTURE_DIR, "dataset.csv")
    holidays_path = joinpath(_GEO_PANEL_FIXTURE_DIR, "holidays.csv")
    ...
    @test only(config.dims) == fixture.panel_dim
    ...
    @test config.adstock["priors"]["alpha"] ==
        EpsilonPrior("Beta"; alpha = 1, beta = 3, dims = ("geo", "channel"))
    ...
    @test config.priors["beta_media"] ==
        EpsilonPrior("HalfNormal"; sigma = 1, dims = ("geo", "channel"))
    @test config.priors["intercept"] ==
        EpsilonPrior("Normal"; mu = 0, sigma = 2, dims = ("geo",))
    @test config.priors["likelihood"].distribution == "Normal"
    @test config.priors["likelihood"].dims == ("date", "geo")
    @test config.priors["likelihood"].parameters[:sigma] ==
        EpsilonPrior("HalfNormal"; sigma = 2, dims = ("geo",))
    ...
    supported_panel_config = ModelConfig(
        ...
        dims = config.dims,
        adstock = copy(config.adstock),
        saturation = copy(config.saturation),
        priors = copy(config.priors),
    )
    supported_panel_model = PanelMMM(supported_panel_config, sampler, data)
    supported_spec = build_model(supported_panel_model)
    supported_runtime = Epsilon._panel_turing_runtime(supported_spec, data)
    @test supported_spec.channel_scale ≈ fixture.channel_scale
    @test supported_spec.target_scale ≈ fixture.target_scale
    @test supported_spec.coordinate_metadata.coordinates["panel_cell"] == fixture.panel_names
    axis = panel_axis(supported_spec)
    @test axis.name == "panel_cell"
    @test axis.values == fixture.panel_names
    @test axis.coordinate_columns == ["geo" => fixture.panel_names]
    @test supported_spec.coordinate_metadata.named_dims["intercept"] == ("geo",)
    @test supported_spec.coordinate_metadata.named_dims["sigma"] == ("geo",)
    @test supported_spec.coordinate_metadata.named_dims["alpha"] == ("geo", "channel")
    @test supported_spec.coordinate_metadata.named_dims["beta_media"] == ("geo", "channel")
    @test supported_runtime.intercept_shape == :panel
    @test supported_runtime.sigma_shape == :panel
    @test supported_runtime.alpha_shape == :channel_panel
    @test supported_runtime.media_beta_shape == :channel_panel
    @test supported_runtime.nintercepts == fixture.npanels
    @test supported_runtime.nsigmas == fixture.npanels
    @test supported_runtime.nalpha == fixture.npanels * length(fixture.channel_columns)
    @test supported_runtime.nmedia_beta == fixture.npanels * length(fixture.channel_columns)
    ...
    @test panel_spec.coordinate_metadata.named_dims["beta_seasonality"] ==
        ("geo", "fourier_mode")
    @test panel_spec.coordinate_metadata.named_dims["holidays"] ==
        ("time", "holiday", "geo")
    ...
    @test any(panel_runtime.holiday_features[:, 1, 2] .> 0)
    @test any(panel_runtime.holiday_features[:, 1, 3] .> 0)
